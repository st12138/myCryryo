...
public class mySM2 {
    ...
    private static byte[] KDF(byte[] Z, int klen) {
        ...
        try {
            for (int i = 1; i < end; i++) {
                baos.write(sm3hash(Z, mySM3.toByteArray(ct)));
                ct++;
            }
            byte[] last = sm3hash(Z, mySM3.toByteArray(ct));
            if (klen % 32 == 0) {
                baos.write(last);
            } else
    ...
    public static void getKey(String id){
        mySM2 sm02 = new mySM2();
        SM2KeyPair keyPair = sm02.generateKeyPair();
        ECPoint publicKey=keyPair.getPublicKey();
        BigInteger privateKey=keyPair.getPrivateKey();
        System.out.println("publickey  for "+id+"\n"+publicKey.toString());
        System.out.println("privatekey for "+id+"\n"+privateKey.toString());
        sm02.exportPublicKey(publicKey, "publickey_"+id+".pem");
        sm02.exportPrivateKey(privateKey, "privatekey_"+id+".pem");
    }

    public static void enKey(String keypath,String pubPath) throws IOException {
        System.out.println("\n-----------------加密对称密钥-----------------");
        //读取密钥
        ...
        mySM2 sm02 = new mySM2();
        ECPoint publicKey = sm02.importPublicKey(pubPath);
        byte[] data = sm02.encrypt(key, publicKey);

        System.out.println("会话密钥："+key);
        System.out.print("加密后密钥("+data.length+")：");
        mySM2.printHexString(data);

        //存放加密后的密钥
        FileOutputStream enkey = new FileOutputStream("enkey");
        enkey.write(data);
        enkey.close();
        System.out.println("公钥加密后的密钥文件保存为 enkey");


    }

    public static void deKey(String enkeyPath,String priPath) throws IOException {
        System.out.println("\n-----------------解密对称密钥-----------------");
        mySM2 sm02 = new mySM2();
        BigInteger privateKey = sm02.importPrivateKey(priPath);

        InputStream getkey = new FileInputStream(enkeyPath);
        byte[] buf = new byte[129];
        getkey.read(buf);
        String key = sm02.decrypt(buf, privateKey);
        System.out.println("解密后密钥:" + key);

        FileOutputStream f = new FileOutputStream("bobKey.txt");
        f.write(key.getBytes());
        f.close();
        System.out.println("对称密钥保存为 bobKey.txt");
    }

    public static Signature mysign(String pubPath,String priPath,String m,String id){
        ...
        Signature signature = sm02.sign(m, id, new SM2KeyPair(publicKey, privateKey));
    ...
    public static  boolean myverify(String m,String id,Signature signature,String pubPath){
        System.out.println("\n-----------------验签-----------------");
        mySM2 sm02 = new mySM2();
        ECPoint publicKey = sm02.importPublicKey(pubPath);
        return sm02.verify(m, signature, id, publicKey);
    }
    public static void main(String[] args) throws IOException {
        Signature signature = mysign("publickey_Alice.pem","privatekey_Alice.pem","123","Alice");
        System.out.println(myverify("123","Alice",signature,"publickey_Alice.pem"));
    }
